"""
ANALYSIS 025 — Tier 1 distance optimization by condition bucket

Question: when is it safe to sell closer than 2.5% OTM on E-0?
Output: condition-bucketed lookup table: under (gap, VIX-proxy, pre-range)
conditions, what distance is safe + expected premium.

Distances tested: 1.5%, 1.75%, 2.0%, 2.25%, 2.5%, 3.0%, 3.5%
Entry: 10:30 IST
Hold: to 15:25 (HOLD strategy)
"""
...
OUT = ROOT / "results" / "025_tier1_distance"
OUT.mkdir(parents=True, exist_ok=True)

con = duckdb.connect()
ENTRY = time(10, 30)
CLOSE = time(15, 25)
DISTANCES = [1.5, 1.75, 2.0, 2.25, 2.5, 3.0, 3.5]
...
def run():
    ...
    # itm rate done differently
    itm = df.groupby(["inst", "regime", "dist_pct"]).apply(
        lambda g: round((g["pe_itm"] | g["ce_itm"]).mean() * 100, 0)
    ).reset_index(name="itm_rate")
    summary = summary.drop(columns="itm_rate").merge(itm, on=["inst","regime","dist_pct"])
